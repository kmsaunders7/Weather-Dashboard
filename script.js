...
// Here we construct our URL using weatherAPI and the city the user inputs
    ...
    $.ajax({
        ...
        // var location = $('<h2>').text(response.name)
        // var date = $('<h3>').text(moment().format('LLL'))
        
        // variable for differnent response calls 
        // var temperature = $('<h4>').text(response.main.temp)
        // var humidity = $('<h4>').text(response.main.humidity)
        // var windSpeed = $('<h4>').text(response.wind.speed)
        // var icon = $('<h4>').text(response.weather[0].icon)
        // $('.currentWeather').append(location, temperature, date, humidity, windSpeed, icon)
